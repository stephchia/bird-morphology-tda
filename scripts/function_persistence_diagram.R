...
persistence_diag <- function(dataset, mya, 
                             ...
                             THRES_PERSISTENCE = 0.4, idx = NULL) {
  ...
  p <- ggplot(dt_H1) +
    ...
    geom_point(subset(dt_H1, persistence < THRES_PERSISTENCE), mapping = aes(x = birth, y = death), 
               shape = 21, size = 1.8, stroke = 0.5, color = cols[2]) +
    geom_point(subset(dt_H1, persistence > THRES_PERSISTENCE), mapping = aes(x = birth, y = death), 
               shape = 21, size = 1.8, stroke = 1, color = cols[2]) +
    geom_point(dt_H1[idx, , drop = F], mapping = aes(x = birth, y = death), 
...
save_trait_pd_plot <- function(dataset, mya, filename) {
  ...
  dir.create("output/img_trait_pd", recursive = TRUE, showWarnings = FALSE)
